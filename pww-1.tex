...
endfig;}
\usepackage{etoc}
\newcommand{\minitoc}{\etocsettocstyle{}{}\localtableofcontentswithrelativedepth{+1}}
%--------------------
...
label.bot(btex \vbox{\openup 6pt\halign{\hss # \hss\cr 
    ...
    $\sin\gamma = {c/2\over1/2} = c$,\enspace $\sin\alpha=a$,\enspace $\sin\beta=b$\cr 
...
label.bot(btex $z=\tan\left(\theta/2\right) \mathbin{\:\Longrightarrow\:} 
...
label.bot("$\displaystyle \sqrt{ab} \le {a+b\over2}$", 
    point 1/2 of bbox currentpicture shifted 42 down);
...
label.bot("$(a+b)^2 - (a-b)^2 = 4ab$", 
    point 1/2 of bbox currentpicture shifted 21 down);
label.bot("$\displaystyle {a+b\over 2} \ge \sqrt{ab}$", 
    point 1/2 of bbox currentpicture shifted 13 down);
...
drawdblarrow aa; 
picture t; t = thelabel("$a+b\over2$", point 1/2 of aa); 
...
label.bot("$AB=a$,\quad $BC=b$,\quad $AD=DC={a+b\over2}$", 
    point 1/2 of bbox currentpicture shifted 42 down);
label.bot("$BE \perp AB$,\quad $DE=AD$", 
    point 1/2 of bbox currentpicture shifted 13 down);
label.bot("$FE \perp ED$,\quad $FB \parallel ED$, \quad $EG=BD={b-a\over2}$", 
    point 1/2 of bbox currentpicture shifted 13 down);
\end{mplibcode}
$$
\vfill
...
label.top(btex $\displaystyle {a\over b} < {c \over d} \quad \Rightarrow \quad
    {a\over b} < {a+c\over b+d} < {c\over d}$ etex, 
    point 5/2 of bbox currentpicture shifted 42 up);
...
label.top(btex $\displaystyle 
    a, b, c, d > 0; \quad {a\over b} < {c \over d} 
...
label.bot(btex $\displaystyle
    {a\over b} < {a\over b+d} + {c \over b+d} < {c\over d}
...
label.top("$\displaystyle 0 < \beta < \alpha < {π \over 2} \implies {\sin \alpha \over \sin \beta} < {\alpha \over \beta} < {\tan \alpha \over \tan \beta}$", 
point 5/2 of bbox currentpicture shifted 42 up);
label.bot("$\sin \alpha < {\sin \beta \over \beta} \alpha $; \quad $\tan \alpha > {\tan \beta \over \beta} \alpha $",
point 1/2 of bbox currentpicture shifted 42 down);
label.bot("$\displaystyle \therefore\quad {\sin \alpha \over \sin \beta} < {\alpha \over \beta} < {\tan \alpha \over \tan \beta}$", 
...
label.top("$T_n = 1 + 2 + \cdots + n$ \quad implies\quad $T_1 + T_2 + \cdots + T_n = \frac16 n(n+1)(n+2)$",
point 5/2 of bbox currentpicture shifted 34 up);

label.bot("$3\left(T_1 + T_2 + \cdots + T_n\right) = T_n \cdot (n+2)$",
...
P11 = image(
    draw P1; 
    label.lft("$1$", p(0, 1/2, 0));
    label.bot("$2$", p(1, 0, 0));
    label.lrt("$3$", p(2, 0, 3/2));
);
P2 = image(
for x = 1 upto 2:
    for y = 0 upto 2:
        for z = 2 downto 0:
            draw c2 shifted p(x, y, z);
        endfor
    endfor
endfor
);
...
P33 = image(
    draw P3; 
    label.lft("$4$", p(0, 2, 0));
    label.bot("$3$", p(3/2, 0, 0));
    label.lrt("$3$", p(3, 0, 3/2));
);
P4 = image(
for x = 1 upto 5:
    for y = 0 upto 3:
        for z = 2 downto 0:
            draw c4 shifted p(x, y, z);
        endfor
    endfor
endfor
);
P44 = image(
    draw P4; 
    label.lft("$n$", p(0, 2, 0));
    label.bot("$n+1$", p(5/2, -1/8, 0));
    label.lrt("$3$", p(5, 0, 3/2));
);
P5 = image(
...
label.top("$3 \bigl(1\times2 + 2\times3 + 3\times4  + \cdots + n(n+1) \bigr) = n (n+1) (n+2)$",
    point 5/2 of bbox currentpicture shifted 34 up);
\end{mplibcode}
$$
\vfill
...
draw P1;
draw P2 shifted (120, -50);
draw P3 shifted (380, -50);

label.top("$\displaystyle \frac{1\cdot2}2 + \frac{2\cdot5}2 + \frac{3\cdot8}2 + \cdots + \frac{n(3n-1)}2 = \frac{n^2(n+1)}2$",
point 5/2 of bbox currentpicture shifted 34 up);
\end{mplibcode}
$$\vfill
...
for k=0 upto 3:
    for i=0 upto n-1:
        for j=i upto n-1:
            fill unitsquare shifted (j + 1/2, i + 1/2) scaled u rotated 90k withcolor Blues 8 3;
        endfor
    endfor
endfor
for i=-n upto n+1:
...
label.bot("$\left(2n+1\right)^2 = 8T_n + 1$", point 1/2 of bbox currentpicture shifted 8 down);
);
P2 = image(
for k=0 upto 3:
    for i=1 upto n-1:
        for j=i upto n-1:
            fill unitsquare shifted (j, i - 1) scaled u rotated 90k withcolor Blues 8 3;
            fill unitsquare shifted (i - 1, j) scaled u rotated 90k withcolor Oranges 8 3;
        endfor
    endfor
endfor
for i=-n upto n:
...
label.bot("$\left(2n\right)^2 = 8T_{n-1} + 4n$", point 1/2 of bbox currentpicture shifted 8 down);
);
interim labeloffset := 13;
label.top(P1, origin);
label.bot(P2, origin);
label.top("$T_n = 1 + 2 + \cdots + n$ \quad \Rightarrow", point 5/2 of bbox
currentpicture shifted 21 up);
\end{mplibcode}
$$
\vfill
...
vardef trig(expr n, shade, edge) = image(
    for y = 1 upto n:
        for x = 1 upto n + 1 - y:
            ...
            draw subpath (1/2, 3/2) of c shifted - center c scaled 3/4 shifted center c withcolor 3/4[shade, white];
            draw c;
        endfor
    endfor
    if edge:
...
label.bot("$3T_n + T_{n-1} = T_{2n}$", point 1/2 of bbox currentpicture shifted 8 down);
);
P2 = image(
    draw t rotated 180;
...
label.bot("$3T_n + T_{n+1} = T_{2n+1}$", point 1/2 of bbox currentpicture shifted 8 down);
);
P3 = image(draw t; draw t shifted 80 up; draw t shifted 80 right;
    ...
    label.bot("$T_{n-1} + 6T_n + T_{n+1} = (2n+1)^2$", point 1/2 of bbox currentpicture shifted 8 down);
);

label.ulft(P1, origin);
label.urt (P2, origin);
label.bot (P3, 21 down);

    label.top("$T_n = 1 + 2 + \cdots + n$ \Longrightarrow", point 5/2 of bbox
    currentpicture shifted 13 up);
\end{mplibcode}
$$
\vfill
...
picture t, u;
numeric n, s; n = 5; s = 7;
path edge; edge = (origin for i=0 upto n-1: -- (-i, n-i) -- (-i-1, n-i) endfor -- (-n, 0) -- cycle) scaled s;
t = image(
    fill edge withcolor Reds 8 2;
    for i=1 upto n:
...
u = image(
    fill edge rotated 180 withcolor Purples 8 2;
    for i=1 upto n:
        draw unitsquare xscaled (n+1-i) yscaled -i scaled s withpen pencircle scaled 1/4;
    endfor
    draw edge rotated 180;
);
picture P[];
    ...
    P[k] = image(
        for i = 1 upto k:
            for j = 1 upto i:
                ...
                else: draw u shifted z shifted (0, n*s) 
                fi;
            endfor
        endfor
        label.bot(if k=1: "$T_n$" else:
            "$" & decimal (k*k) & "T_n + " & decimal floor(k*k/8) & "=T_{" & decimal k & "n+" & decimal floor(k/2) & "}$" fi,
        point 1/2 of bbox currentpicture);
    );
endfor

draw P1;
draw P3 shifted (70, 0);
draw P5 shifted (175, -40);
draw P7 shifted (300, -140);

label.bot("$\left(2k+1\right)^2T_n + T_k = T_{(2k+1)n+k}$", point 1/2 of bbox currentpicture shifted 21 down);
...
for i=1 upto 4:
    ball[i] = image(
        fill fullcircle scaled u withcolor Spectral[4][i];
    ...
    for i = 1 upto 9:
        for j = 1 upto i:
            draw ball
                ...
                shifted ((j - 1/2 i, -0.866025 i) * 1.44u);
        endfor
    endfor
...
P3 = image(
    for i=1 upto 9:
        for j=1 upto 5:
            draw ball[
                if i < 6:
                    if i + j < 7: 1 else: 2 fi
                else:
                    if i + j < 11: 3 else: 4 fi
                fi
            ] shifted ((i, j) * 1.44u);
        endfor
    endfor
...
T_n&1+2+\cdots+n\cr}}\right\} \Longrightarrow 
H_n = 3T_{n-1} + T_n = T_{2n-1} = n(2n-1)$
etex, point 5/2 of bbox currentpicture shifted 34 up);
\end{mplibcode}
$$
\vfill
...
vardef frame(expr N, Closed) = image(
    for i = 0 upto N-1:
        for j = 0 upto N-1:
            path s; 
            s = unitsquare shifted (i, j) scaled u;
            if j > i + floor(N / 2): 
                if not Closed:
    ...
    for i = 0 upto N:
        numeric minx, maxy;
        minx = if Closed or (i < floor(N / 2) + 1): 0 else: i - floor(N / 2) fi;
        maxy = if Closed: N else: i + floor (N / 2) if i < N: + 1 fi fi;
    ...
    if not Closed:
        ...
        draw ((0, m) -- (1, m)
            for i = 1 upto N - 1:
                -- (i, m+i) -- (i+1, m+i)
                hide(if (m + i) > N: 
                    draw ((i+1, m+i) -- (N, m+i)) scaled u marked(m+i, N);
                fi)
            endfor
            ) scaled u withpen pencircle scaled 3/4;
    fi
)
enddef;
numeric N, x, y; N = 3; x = 0; y = -45u;
for n=0 upto N:
    picture f; 
    f = frame(3**n, false);
    draw f shifted (x, y);
    x := x + 28 + xpart lrcorner f;
endfor
numeric x, y; x = 0; y = 0;
for n=0 upto N:
    picture f; 
...
vardef hexagon(expr N, Label) = image(
    ...
    for n = 1 upto N - 1:
        for r = 0 upto 5:
            for k = 1 upto n:
                path d; 
                d = disc shifted (k/n)[(0, n * u) rotated 60r, (0, n * u) rotated (60r + 60)];
...
label.top(P1, 13 up);
label.bot(P2, 13 down);

label.bot("$h_n = n^3 - (n-1)^3$", point 1/2 of bbox currentpicture shifted 13 down);
label.bot("$\therefore \quad h_1 + h_2 + \cdots + h_n = n^3$", point 1/2 of bbox currentpicture shifted 13 down);
...
m1 = xx scaled 2 rotated angle z1 cutbefore subpath (-1, 1) of bbox ff cutafter subpath (1,3) of bbox ff;
m2 = xx scaled 2 rotated angle z2 cutbefore subpath (-1, 1) of bbox ff cutafter (point 1 of m0 -- point 1 of m1);
...
draw ff withcolor Reds 8 6;    label.top("$y=\frac{x}{\ln(x)}$", point infinity of ff) withcolor Reds 8 7;
draw xy withcolor Oranges 8 6; label.top("$y=x$",                point infinity of xy) withcolor Oranges 8 7;
drawarrow xx;
drawarrow yy;
...
label.bot("$\displaystyle x_0 > 1 \enspace\mathbin{\&}\enspace x_{n+1} = \frac{x_n}{\ln(x_n)} \quad\mathbin{\Longrightarrow}\quad \lim x_n = e$", 
point 1/2 of bbox currentpicture shifted 34 down);
    ...
    for i=1 upto n:
        ...
        if sf > 1/4:
            if r = 1/2:
                ...
                label(if i=1: "$\frac12$" else: TEX("$\frac12^" & decimal (2i-1) & "$") scaled sf fi, center a);
            else:
...
{a + ar + ar^2 + ar^3 + ar^4 + \cdots \over 1/r}&{ar\over 1-r}\cr
 a + ar + ar^2 + ar^3 + ar^4 + \cdots &{a\over 1-r}\cr
\sum_{n=0}^\infty ar^n&{a\over 1-r}\cr
...
\therefore\quad 1 + r + r^2 + r^3 + \cdots = {1\over 1-r}\cr}} etex,
point 1/2 of bbox currentpicture shifted 21 down);
\end{mplibcode}
$$
\vfill
...
numeric u; u = 160;
    ...
    for i = 0 upto 9:
        ...
        draw s; draw t;
        draw t rotatedabout(center s, 90);
        if i < 3:
            picture q, p;
            if i=0:
                q = TEX("$1-r$"); p = TEX("$r$");
            elseif i=1:
                q = TEX("$r(1-r)$") scaled 0.8;
                p = TEX("$r^2$") scaled 0.8;
            else:
                q = TEX("$r^2(1-r)$") scaled 0.45;
                p = TEX("$r^3$") scaled 0.45;
            fi
            label.top(q, point 1/2 of s - (0, 1/2i));
            label.top(p, point 1/2 of t - (0, 1/2i));
        fi
    endfor
    label.bot(btex \vbox{\openup 13pt\halign{\hss $\displaystyle # + \cdots$&$\displaystyle {}=#$\hss\cr
    (1-r)^2 + r^2(1-r)^2 + r^4(1-r)^2 & {(1-r)^2\over (1-r)^2 + 2\times r(1-r)}\cr
    1 + r^2 + r^4 & {1 \over (1 - r)^2 + 2r(1-r)} = {1\over 1-r^2} \cr
    a + ar + ar^2 & {a \over 1-r} \cr
    }} etex, point 1/2 of bbox currentpicture shifted 13 down);
);
label.top(P1, 7 up);
label.bot(P2, 7 down);
...
vardef label_bar(expr a, b, z, t) = 
    label.rt(t, z);
    drawarrow z {left} .. {left} 1/2[a, b]
        cutafter fullcircle scaled dotlabeldiam shifted 1/2[a, b]
        withpen pencircle scaled 1/4;
enddef;

label_bar((n * u, 0), (n * u, v / (n + 1)), (n * u + 1/2 u, v / (n + 1) - 12), "$\displaystyle\frac{1}{n+1}$");
label_bar((u, (n-1)*v/n), (u, n*v/(n+1)), (3/2 u, n*v/(n+1) + 4),
"$\displaystyle\frac{1}{n}\cdot\frac1{n+1}$");
%"$\displaystyle\frac{n}{n+1}-\frac{n-1}{n}=\frac{n^2 - (n^2 - 1)}{n(n+1)}=\frac1{n(n+1)}$");

label.bot("$\displaystyle \frac1{1\cdot2} + \frac1{2\cdot3} + \cdots + \frac1{n(n+1)} = \frac{n}{n+1}$",
    point 1/2 of bbox currentpicture shifted 21 down);
\end{mplibcode}
$$\vfill
...
ff = ((minx - s, 2/(minx - s)) for x = minx step s until maxx + 2s: .. (x, 2/x) endfor) xscaled u yscaled v;
...
for x = minx upto maxx - 1:
    ...
    if x = maxx - 1:
        label.top("\vbox{\openup4pt\halign{\hss $#$ \hss\cr1\cdot\left(\frac2n-\frac2{n+1}\right)\cr=\frac2{n(n+1)}\cr}}", point 1/2 of b shifted 8 up);
...
ff = (15/16, 16/15) for x = 1 step 1/8 until 9/4: .. (x, 1/x) endfor;
ff := ff scaled u;

xx = (xpart point 0 of ff, 0) -- (xpart point infinity of ff, 0);
yy = (point 0 of xx -- point 0 of ff) shifted 21 left;

draw ((1, 0) -- (1, 1)) scaled u;
draw ((2, 0) -- (2, 1/2)) scaled u;

vardef gcd(expr a, b) = if b = 0: a else: gcd(b, a mod b) fi enddef;
vardef reduced_fraction(expr N, D) = 
    save n, d, g; numeric n, d, g;
    g = gcd(N, D);
    n = N / g;
    d = D / g;
    if d = 1:
        ...
        "$\frac{" & decimal n & "}{" & decimal d & "}$"
    fi
enddef;
vardef fh(expr a) = 
    save s, t;
    for n = 1 upto 8:
        numeric t; t = 1/n - 1/(n+1);
        string s; s = "$" if n>1: & "\frac1" fi & decimal n & "-" & "\frac1" & decimal (n+1) & "$";
        exitif abs(a-t) < eps;
    endfor
    s
enddef;
...
label.bot("$\ln2=1-\frac12+\frac13-\frac14+\frac15-\frac16+\cdots$", point 5/2 of bbox currentpicture);
label.bot(btex
...
\ln 2 = \int_1^2 \frac1x\,dx 
      = \left(1-\frac12\right) + \left(\frac13-\frac14\right) + \cdots
...
label.top("$\displaystyle\sin\,(2n+1)\theta = \sin\theta + 2\sin\theta \sum_{k=1}^{n} \cos 2k\theta$",
            point 5/2 of bbox currentpicture shifted 21 up);
...
fill t1 withcolor Oranges 8 3;
fill t2 withcolor Oranges 8 2;
fill t3 withcolor Blues 8 3;
fill t4 withcolor Blues 8 2;
fill t7 withcolor Reds 8 3;
fill t8 withcolor Reds 8 3;
fill t5 withcolor Greens 8 3;
fill t6 withcolor Greens 8 3;
draw t1;
draw t2;
draw t3;
draw t4;
...
t = TEX("$\left|\,\vcenter{\halign{\hss$#$\hss&\quad\hss$#$\hss\cr a&b\cr c&d\cr}}\,\right|=ad-bc={}$");
...
P3 = image(
    fill unitsquare xscaled (c-x3) yscaled b 
        shifted z3
        withcolor Blues 8 2;
    drawoptions(dashed withdots scaled 1/2);
    draw (0, b) -- (a, b);
...
P4 = image(
    fill unitsquare xscaled x3 yscaled (d-b) 
        shifted (0, b)
        withcolor Blues 8 2;
    drawoptions(dashed withdots scaled 1/2);
    draw (x3, b) -- (x3, d);
    ...
    label.bot("$a$", (a, 0));
    label.bot("$c$", (c, 0));
    label.lft("\llap{$b$}", (0, b));
    label.lft("\llap{$d$}", (0, d));
);

interim labeloffset := 24;
label.ulft(P1, origin);
label.urt (P2, origin);
label.llft(P3, origin);
label.lrt (P4, origin);
